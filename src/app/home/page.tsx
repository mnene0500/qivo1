  ...
  const fetchUsers = useCallback(async (pageNum = 0) => {
    ...
    try {
      ...
      if (activeTab === 'recommend') {
        // Randomish order via updated_at desc + limit
        query.order('updated_at', { ascending: false });
      } else {
        // Strict activity based order for "Nearby" feel
        query.order('updated_at', { ascending: false });
      }
      ...
      <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-xl px-5 py-3 flex items-center justify-between border-b border-gray-50 h-14">
        <div className="flex items-center gap-6">
          {['Recommend', 'Nearby'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab.toLowerCase() as any)} className={cn("text-xs font-black uppercase tracking-widest transition-all", activeTab === tab.toLowerCase() ? "text-[#00A2FF] scale-110" : "text-gray-300 hover:text-black")}>
              {tab}
            </button>
          ))}
        </div>
