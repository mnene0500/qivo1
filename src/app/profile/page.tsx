...
function JoinAgencyDialog({ userUid }: { userUid: string }) {
  ...
  const handleJoin = async () => {
    ...
    if (res.success) { toast({ title: "Success", description: "Application sent!" }); setOpen(false); }
    else toast({ variant: "destructive", title: "Error", description: res.error })
    setLoading(false)
  }
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button className="h-20 bg-pink-500 rounded-2xl shadow-xl flex flex-col items-center justify-center gap-1 text-white col-span-2 mt-4"><UserPlus className="w-6 h-6" /><span className="text-[10px] font-bold uppercase tracking-widest">Join Agency</span></Button></DialogTrigger>
      <DialogContent className="rounded-3xl p-8 max-w-[90vw]"><DialogHeader className="items-center text-center"><DialogTitle className="text-xl font-bold">Enter Agency Code</DialogTitle><DialogDescription className="text-xs">Ask your agent for their 5-digit code.</DialogDescription></DialogHeader>
        <div className="py-6"><Input maxLength={5} placeholder="e.g. 54321" value={code} onChange={(e) => setCode(e.target.value)} className="rounded-2xl h-16 text-center text-2xl font-bold tracking-[0.5em]" /></div>
        <Button onClick={handleJoin} disabled={loading || code.length !== 5} className="w-full h-14 bg-pink-500 rounded-full font-bold uppercase">Apply Now</Button>
      </DialogContent>
    </Dialog>
  )
}
...
function AgencyDashboardDialog({ user }: { user: UserProfile }) {
  ...
  const handleCreate = async () => {
    ...
    if (res.success) toast({ title: "Agency Created", description: `Code: ${res.code}` })
    else toast({ variant: "destructive", title: "Error", description: res.error })
    setLoading(false)
  }
  return (
