...
export async function sendMysteryNoteAction(senderUid: string, text: string, count: number) {
  ...
  try {
    ...
    const { data: targets } = await supabase.from('users').select('uid').eq('gender', targetGender).eq('onboarding_complete', true).neq('uid', senderUid).limit(count);

    if (targets) {
      for (const t of targets) {
        const chatId = `direct_${[senderUid, t.uid].sort()[0]}_${[senderUid, t.uid].sort()[1]}`;
        await supabase.from('chats').upsert({ 
          id: chatId, last_message: text.slice(0, 100), last_message_at: ts, 
          participant_ids: [senderUid, t.uid], last_sender_id: senderUid, updated_at: new Date().toISOString()
        });
...
export async function awardCoinsAction(ownerUid: string, targetUid: string, amount: number) {
  ...
  try {
    ...
    await supabase.from('coin_history').insert({ user_id: targetUid, amount, type: 'awarded', description: `Received from Admin`, timestamp: Date.now() });
